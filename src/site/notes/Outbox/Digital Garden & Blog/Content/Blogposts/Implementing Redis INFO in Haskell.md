...
In a [[Outbox/Digital Garden & Blog/Content/Blogposts/Yes - Cabal custom scripts do have uses\|previous blog post]], I talked about how I used Cabal's custom setup script to generate a module with build info at compile time. As mentioned in that post, the custom setup script work was but a piece of what went into implementing the `INFO` command for my Redis server. Let's talk about it.
